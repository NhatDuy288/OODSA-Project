...
function Notifications({ onClick }) {
    ...
    // NOTE: không resolve ở đây nữa, Avatar component sẽ tự resolve
    const enrichNotificationAvatar = async (n) => {
        if (!n) return n;

        const existed =
            n?.senderAvatar ?? n?.avatar ?? n?.sender?.avatar ?? n?.user?.avatar ?? n?.sender?.avatarUrl ?? n?.user?.avatarUrl;
    ...
    const handleGetAllNotification = async () => {
        try {
            ...
            const enriched = await Promise.all((res || []).map(enrichNotificationAvatar));
            ...
            <div onClick={(e) => e.stopPropagation()} className={styles.wrapper}>
                ...
                <div className={styles.content}>
                    ...
                    <div className={styles.notifications}>
                        ...
                        {!isLoading && notifications.length === 0 && <div>Không có thông báo</div>}
